...
class ReflexVacuumAgent:
    """
    Agente reactivo simple para el mundo de la aspiradora.
    Equivalent NetLogo: turtle con reglas condicion-accion
    Equivalent ESP32:   logica de decision en loop()
    """

    def __init__(self, location):
        """
        Inicializa el agente en una ubicacion dada.
        Equivalent NetLogo: create-turtles
        Equivalent ESP32:   variables globales del agente en setup()
        """
    ...
    def act(self, env):
        """
        Decide y ejecuta una accion basada en la percepcion actual.
        Equivalent NetLogo: reglas condicion-accion de la tortuga
        Equivalent ESP32:   logica en loop()
        """
        location, status = self.percept(env)
        if status == 'Dirty':
            action = 'Suck'
            self.performance += 1
        elif location == env.loc_A:
            action = 'Right'
        else:
            action = 'Left'
...
if __name__ == '__main__':
    ...
    agent = ReflexVacuumAgent(location=(0, 0))
